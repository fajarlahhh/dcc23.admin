...
class Requestwd extends Component
{
    ...
    public function render()
    {
        return view('livewire.requestwd', [
            'i' => 0,
            'data' => Withdrawal::with('user')->when($this->status == 1, fn ($q) => $q->whereNull('processed_at'))->when($this->status == 2, fn ($q) => $q->where('processed_at', 'like', $this->year . '-' . $this->month . '%'))->orderBy('to_wallet')->get(),
        ]);
    }
}
